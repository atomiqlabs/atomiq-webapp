...
- `src/components/layout/SiteFooterView.tsx` — the full footer: link columns (Product / Developers / Legal), a curated set of swap-route links, and the existing socials row (embeds `SocialFooterView` rather than duplicating it). Takes an `appOrigin` prop: `''` (default) on the `/swap` pages for subdomain-relative links, `'https://app.atomiq.exchange'` on the apex landing so app links are absolute.
...
2. Hero — headline + subhead + primary CTA "Start a swap" → `https://app.atomiq.exchange/`.
...
5. How it works — three steps: connect both wallets → get an RFQ quote from market makers → atomic settlement, keep custody throughout.
