    ...
    return (
        <div className="d-flex my-2">
            <span className="d-flex align-items-center">
                {props.text}
                {props.composition==null ? "" : props.composition.base==null ? (
                    <Badge bg="primary" className="ms-1 pill-round px-2" pill>{props.composition.percentage.percentage} %</Badge>
                ) : (
            ...
            <span className="ms-auto">{props.isApproximate? "~" : ""}{toHumanReadableString(props.amount, props.token)} {props.token.ticker}</span>
        </div>
    );
}
