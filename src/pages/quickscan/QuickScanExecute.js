...
export function QuickScanExecute() {
  ...
  const [autoContinue, setAutoContinue] = useLocalStorage(
    "crossLightning-autoContinue",
    false,
  );
  const [addressResult, addressLoading, addressError] =
    useAddressData(propAddress);
  const inToken =
    ...
    addressResult?.swapType == null
      ...
      : addressResult.swapType === SwapType.TO_BTCLN
        ...
        : addressResult.swapType === SwapType.TO_BTC
          ? Tokens.BITCOIN.BTC
          : selectedCurrency;
  const [amount, setAmount] = useStateWithOverride(
    null,
    addressResult?.amount?.amount,
  );
  const exactIn =
    addressResult?.swapType === SwapType.FROM_BTCLN ||
  ...
  const { input: inputLimit, output: outputLimit } = useAmountConstraints(
    inToken,
    outToken,
  );
  ...
  const amountConstraints = useMemo(() => {
    return {
      min: BigNumber.max(
        btcConstraints?.min ?? 0,
        new BigNumber(addressResult?.min?.amount) ?? 0,
      ),
      max: BigNumber.min(
  ...
  const amountValidator = useCallback(
    numberValidator(amountConstraints, true),
    [amountConstraints],
  );
  ...
  return _jsxs(_Fragment, {
    children: [
      _jsx(SwapTopbar, { selected: 1, enabled: !isLocked }),
      _jsx("div", {
        className:
          "d-flex flex-column flex-fill justify-content-center align-items-center text-white",
        children: _jsxs("div", {
          className: "quickscan-summary-panel d-flex flex-column flex-fill",
          children: [
            _jsxs("div", {
              className: "p-3 d-flex flex-column tab-bg border-0 card",
              children: [
                _jsx(ValidatedInput, {
                  type: "text",
                  className: "",
                ...
                _jsx(ErrorAlert, {
                  className: "mt-3",
                  title: "Destination parsing error",
                  error: addressError,
                }),
                addressLoading
                  ? _jsxs("div", {
                      className:
                        "d-flex flex-column align-items-center justify-content-center tab-accent mt-3",
                      children: [
                ...
                addressError == null && swapper != null && !addressLoading
                  ? _jsxs("div", {
                      className: "mt-3 tab-accent-p3 text-center",
                      children: [
                        _jsx("label", {
                          className: "fw-bold mb-1",
                          children: !exactIn ? "Pay" : "Withdraw",
                        }),
                        _jsx(ValidatedInput, {
                          type: "number",
                          textEnd: _jsxs("span", {
                            className:
                              "text-white font-bigger d-flex align-items-center",
                            children: [
                              _jsx(TokenIcon, {
                                tokenOrTicker: btcToken,
                                className: "currency-icon",
                              }),
                              "BTC",
                            ],
                          }),
                          step: new BigNumber(10).pow(
                            new BigNumber(-btcToken.decimals),
                          ),
                        ...
                        _jsx("label", {
                          className: "fw-bold mb-1",
                          children: !exactIn ? "with" : "to",
                        }),
                        _jsx("div", {
                          className: "d-flex justify-content-center",
                          children: _jsx(CurrencyDropdown, {
                        ...
                        _jsxs(Form, {
                          className:
                            "text-start d-flex align-items-center justify-content-center font-bigger mt-2",
                          children: [
                            _jsx(
                              Form.Check, // prettier-ignore
                              {
                                ...
                                type: "switch",
                                onChange: (val) =>
                                  setAutoContinue(val.target.checked),
                                checked: autoContinue,
                              },
                            ),
                            _jsx("label", {
                              title: "",
                              htmlFor: "autoclaim-pay",
                              className: "form-check-label me-2",
                              children: !exactIn ? "Auto-pay" : "Auto-claim",
                            }),
                            _jsx(OverlayTrigger, {
                ...
                quote != null
                  ? _jsxs(_Fragment, {
                      children: [
                        _jsx(FeeSummaryScreen, {
                          swap: quote,
                          className: "mt-3 mb-3 tab-accent",
                        }),
                        _jsx(QuoteSummary, {
                          ...
                          autoContinue:
                            autoContinue &&
                            (!isCurrencyPreselected ||
                              addressResult?.amount != null),
                        }),
                      ],
                    })
                  : "",
                _jsx(ScrollAnchor, { trigger: quote != null }),
              ],
            }),
            _jsx("div", {
              className: "d-flex mt-auto py-4",
              children: _jsx(Button, {
                variant: "secondary flex-fill",
                disabled: isLocked,
                onClick: goBack,
                children: "< Back",
              }),
