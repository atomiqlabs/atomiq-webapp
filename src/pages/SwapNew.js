...
export function SwapNew(props) {
    ...
    let [addressData, addressLoading, addressError] = useAddressData(address, (addressData) => {
        ...
        if (outputChainData.wallet != null && outputChainData.wallet.address != null && outputChainData.wallet.address !== addressData.address) {
    ...
    if (outputChainData?.wallet?.address != null) {
        addressData = { address: outputChainData.wallet.address, type: outputChainData.id, swapType: null };
    ...
    const [outputAddress, inputAmount, outputAmount, isOutputWalletAddress] = useMemo(() => {
        if (existingSwap != null)
            return [existingSwap.getOutputAddress(), existingSwap.getInput().amount, existingSwap.getOutput().amount, outputChainData?.wallet?.address === existingSwap.getOutputAddress()];
        let _address = outputChainData?.wallet?.address ?? address;
        if (quote != null)
            return [randomQuote ? _address : quote.getOutputAddress(), exactIn ? amount : quote.getInput().amount, !exactIn ? amount : quote.getOutput().amount, outputChainData?.wallet?.address === quote.getOutputAddress()];
    ...
    }, [exactIn, amount, existingSwap, quote, randomQuote, address, outputChainData?.wallet?.address]);
    const notEnoughBalance = quote != null && maxSpendable?.balance != null && quote.getInput().rawAmount > maxSpendable.balance.rawAmount;
    ...
    return (_jsxs(_Fragment, { children: [_jsx(SwapTopbar, { selected: 0, enabled: !locked }), _jsx(QRScannerModal, { onScanned: (data) => {
                    ...
                    setQrScanning(false);
                                        ...
                                        }, inputId: "amount-output", inputClassName: "font-weight-500", floatingLabel: outputValue == null ? null : FEConstants.USDollar.format(outputValue), expectingFloatingLabel: true, step: outputToken == null ? new BigNumber("0.00000001") : new BigNumber(10).pow(new BigNumber(-(outputToken.displayDecimals ?? outputToken.decimals))), min: outputLimits?.min, max: outputLimits?.max, onValidate: outputAmountValidator, validated: (exactIn && quote != null) || existingSwap != null ? null : undefined, elementEnd: (_jsx(CurrencyDropdown, { currencyList: outputTokens, onSelect: (val) => {
                                                if (locked)
                                                    return;
                                                leaveExistingSwap();
                                                if (val === outputToken)
                                            ...
                                            leaveExistingSwap();
                                                ...
                                                if (!forcedChange)
                                                    ...
                                                    outputChainData.connect();
                                                        ...
                                                        e.preventDefault();
                                                        setQrScanning(true);
                                                    }, children: _jsx(Icon, { size: 24, icon: ic_qr_code_scanner }) }) }))), successFeedback: isOutputWalletAddress ? "Address fetched from your " + outputChainData?.wallet.name + " wallet!" : null }), webLnForOutput ? (_jsx(_Fragment, { children: addressData?.address == null && validatedAmount != null ? (_jsx("div", { className: "mt-2", children: _jsx("a", { href: "#", onClick: async (e) => {
                                                        e.preventDefault();
