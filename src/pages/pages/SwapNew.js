...
import { isBtcToken, isSCToken, SpvFromBTCSwap, SwapType, toTokenAmount, fromHumanReadableString, } from '@atomiqlabs/sdk';
...
export function SwapNew(props) {
    ...
    // Automatically fetch invoice from WebLN when amount is set
    useEffect(() => {
        if (!webLnForOutput)
            return;
        ...
        const fetchInvoice = async () => {
            ...
            try {
                ...
                console.error('Failed to fetch invoice from WebLN:', e);
            }
        };
        fetchInvoice();
    }, [webLnForOutput, validatedAmount, addressData?.address, outputChainData]);
