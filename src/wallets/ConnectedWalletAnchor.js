...
export function ConnectedWalletAnchor({ className, noText, currency, variantButton = 'transparent', simple = false, maxSpendable, setMax, }) {
    ...
    if (simple && wallet != null) {
        return (_jsxs("div", { className: "wallet-connections wallet-connections__simple", children: [_jsx("img", { width: 16, height: 16, src: wallet.icon, alt: wallet.name }), !isLightning && (_jsx(_Fragment, { children: maxSpendable?.amount ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "wallet-connections__amount", children: [maxSpendable.amount, " ", currency.ticker] }), setMax ? (_jsx(BaseButton, { variant: "border-only", className: "wallet-connections__simple__max", onClick: setMax, children: "max" })) : null] })) : (_jsx("div", { className: "wallet-connections__amount is-loading" })) })), _jsx("div", { className: "wallet-connections__simple__disconnect", children: _jsx(OverlayTrigger, { overlay: _jsx(Tooltip, { children: "Disconnect wallet" }), children: _jsx("div", { className: "icon icon-disconnect", onClick: () => disconnectWallet(chainId) }) }) })] }));
    }
