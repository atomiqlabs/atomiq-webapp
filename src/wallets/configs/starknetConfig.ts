 ...
 */
function waitTillAddressPopulated(acc: WalletAccount): Promise<void> {
...
/**
 * Starknet-specific wallet data
 * We need both SWO and WalletAccount
 */
 ...
 * Starknet chain configuration
 */
export const starknetConfig: ChainConfig<StarknetWalletData, StarknetSigner, never> = {
  // ========== Metadata ==========
  id: 'STARKNET',
  name: 'Starknet',
  ...
  disconnectWallet: async (walletData) => {
    await disconnect({ clearLastWallet: true }).catch((e) =>
      console.error('[Starknet] Error during disconnect:', e)
    );
