...
export function FromBTCQuoteSummary(props) {
    ...
    useEffect(() => {
        ...
        if (state === FromBTCSwapState.CLAIM_COMMITED || state === FromBTCSwapState.EXPIRED) {
            ...
            props.quote.waitForBitcoinTransaction(abortController.signal, null, (txId, confirmations, confirmationTarget, txEtaMs) => {
    ...
    return (_jsxs(_Fragment, { children: [(!isCreated || commitLoading) && !isQuoteExpired ? _jsx(StepByStep, { steps: executionSteps }) : "", _jsx(SwapExpiryProgressBar, { expired: isQuoteExpired, timeRemaining: quoteTimeRemaining, totalTime: totalQuoteTime, show: (isCreated || isQuoteExpired) && !commitLoading && !props.notEnoughForGas && signer !== undefined && hasEnoughBalance }), isCreated && hasEnoughBalance ? (signer === undefined ? (_jsx(ButtonWithSigner, { chainId: props.quote.chainIdentifier, signer: signer, size: "lg" })) : (_jsxs(_Fragment, { children: [_jsx(SwapForGasAlert, { notEnoughForGas: props.notEnoughForGas, quote: props.quote }), _jsxs(Alert, { className: "text-center mb-3", show: commitError != null, variant: "danger", closeVariant: "white", children: [_jsx("strong", { children: "Swap initialization error" }), _jsx("label", { children: commitError?.message })] }), _jsxs(ButtonWithSigner, { signer: signer, chainId: props.quote.chainIdentifier, onClick: onCommit, disabled: commitLoading || !!props.notEnoughForGas || !hasEnoughBalance, size: "lg", className: "d-flex flex-row", children: [commitLoading ? _jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" }) : "", "Initiate swap"] })] }))) : "", isCommited ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "mb-3 tab-accent", children: walletConnected != null ? (_jsxs(_Fragment, { children: [_jsxs(Alert, { variant: "danger", className: "mb-2", show: !!payError, children: [_jsx("strong", { children: "Sending BTC failed" }), _jsx("label", { children: payError })] }), _jsx("div", { className: "d-flex flex-column align-items-center justify-content-center", children: payTxId != null ? (_jsxs("div", { className: "d-flex flex-column align-items-center tab-accent", children: [_jsx(Spinner, {}), _jsx("label", { children: "Sending Bitcoin transaction..." })] })) : (_jsxs(_Fragment, { children: [_jsxs(Button, { variant: "light", className: "d-flex flex-row align-items-center", disabled: payLoading, onClick: sendBitcoinTransaction, children: [payLoading ? _jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" }) : "", "Pay with ", _jsx("img", { width: 20, height: 20, src: walletConnected.getIcon(), className: "ms-2 me-1" }), " ", walletConnected.getName()] }), _jsx("small", { className: "mt-2", children: _jsx("a", { href: "javascript:void(0);", onClick: disconnect, children: "Or use a QR code/wallet address" }) })] })) })] })) : (_jsx(CopyOverlay, { placement: "top", children: (show) => (_jsxs(_Fragment, { children: [_jsx(Alert, { variant: "warning", className: "mb-3", children: _jsxs("label", { children: ["Please make sure that you send an ", _jsx("b", { children: _jsx("u", { children: "EXACT" }) }), " amount in BTC, different amount wouldn't be accepted and you might loose funds!"] }) }), _jsx("div", { className: "mb-2", children: _jsx(QRCodeSVG, { value: props.quote.getQrData(), size: 300, includeMargin: true, className: "cursor-pointer", onClick: (event) => {
