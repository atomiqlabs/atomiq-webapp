...
export function FromBTCQuoteSummary(props) {
    ...
    const addressContent = useCallback((show) => (_jsxs(_Fragment, { children: [_jsx(Alert, { variant: "warning", className: "mb-3", children: _jsxs("label", { children: ["Please make sure that you send an ", _jsx("b", { children: _jsx("u", { children: "EXACT" }) }), " amount in BTC, different amount wouldn't be accepted and you might lose funds!"] }) }), _jsx("div", { className: "mb-2", children: _jsx(QRCodeSVG, { value: props.quote.getQrData(), size: 300, includeMargin: true, className: "cursor-pointer", onClick: (event) => {
                    ...
                    } }) }), _jsxs("label", { children: ["Please send exactly ", _jsx("strong", { children: props.quote.getInput().amount }), " ", Tokens.BITCOIN.BTC.ticker, " to the address"] }), _jsx(ValidatedInput, { type: "text", value: props.quote.getBitcoinAddress(), textEnd: (_jsx("a", { href: "#", onClick: (event) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(OnchainAddressCopyModal, { openRef: openModalRef, amountBtc: props.quote?.getInput()?.amount, setShowCopyWarning: setShowCopyWarning }), isInitiated ? _jsx(StepByStep, { steps: executionSteps }) : "", _jsx(SwapExpiryProgressBar, { expired: isQuoteExpired, timeRemaining: quoteTimeRemaining, totalTime: totalQuoteTime, show: (isCreated || isQuoteExpired) && !commitLoading && !props.notEnoughForGas && signer !== undefined && hasEnoughBalance }), _jsx(ErrorAlert, { className: "mb-3", title: "Swap initialization error", error: commitError }), isCreated && hasEnoughBalance ? (signer === undefined ? (_jsx(ButtonWithSigner, { chainId: props.quote.chainIdentifier, signer: signer, size: "lg" })) : (_jsxs(_Fragment, { children: [_jsx(SwapForGasAlert, { notEnoughForGas: props.notEnoughForGas, quote: props.quote }), _jsxs(ButtonWithSigner, { signer: signer, chainId: props.quote.chainIdentifier, onClick: onCommit, disabled: commitLoading || !!props.notEnoughForGas || !hasEnoughBalance, size: "lg", className: "d-flex flex-row", children: [commitLoading ? _jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" }) : "", "Initiate swap"] })] }))) : "", isCommited ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "mb-3 tab-accent", children: walletConnected != null ? (_jsxs(_Fragment, { children: [_jsx(ErrorAlert, { className: "mb-2", title: "Sending BTC failed", error: payError }), _jsx("div", { className: "d-flex flex-column align-items-center justify-content-center", children: payTxId != null ? (_jsxs("div", { className: "d-flex flex-column align-items-center p-2", children: [_jsx(Spinner, {}), _jsx("label", { children: "Sending Bitcoin transaction..." })] })) : (_jsxs(_Fragment, { children: [_jsxs(Button, { variant: "light", className: "d-flex flex-row align-items-center", disabled: payLoading, onClick: sendBitcoinTransactionRef.current, children: [payLoading ? _jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" }) : "", "Pay with ", _jsx("img", { width: 20, height: 20, src: walletConnected.getIcon(), className: "ms-2 me-1" }), " ", walletConnected.getName()] }), _jsx("small", { className: "mt-2", children: _jsx("a", { href: "#", onClick: (e) => {
                                                        e.preventDefault();
                                                        disconnect();
