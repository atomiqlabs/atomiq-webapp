...
export function ToBTCSwapPanel(props) {
    ...
    const swapFees = (_jsx("div", { className: "mt-3", children: _jsx(SwapFeePanel, { swap: props.quote, isExpired: page.step4?.state === 'expired', onRefreshQuote: props.refreshQuote, totalTime: page.step1init?.expiry.total, remainingTime: page.step1init?.expiry.remaining }) }));
    if (page.step1init) {
    ...
    if (page.step2paying) {
        return (_jsx(_Fragment, { children: _jsxs("div", { className: "swap-panel__card", children: [stepByStep, _jsx(SwapStepAlert, { show: !!page.step2paying.error, type: "error", title: page.step2paying.error?.title, error: page.step2paying.error?.error, actionElement: page.step2paying.error?.retry && (_jsxs(BaseButton, { className: "swap-step-alert__button", onClick: page.step2paying.error?.retry, variant: "secondary", children: [_jsx("i", { className: "icon icon-retry" }), "Retry"] })) })] }) }));
    }
    if (page.step3refund) {
        return (_jsx(_Fragment, { children: _jsxs("div", { className: "swap-panel__card", children: [stepByStep, _jsx(SwapStepAlert, { show: !!page.step3refund.error, type: "error", icon: ic_warning, title: page.step3refund.error?.title, error: page.step3refund.error?.error }), _jsx(SwapStepAlert, { type: "danger", icon: ic_error_outline_outline, title: "Swap failed", description: "Swap failed, you can refund your prior deposit", actionElement: _jsxs(ButtonWithWallet, { className: "swap-step-alert__button", requiredWalletAddress: props.quote._getInitiator(), chainId: props.quote.chainIdentifier, onClick: page.step3refund.refund?.onClick, disabled: page.step3refund.refund?.disabled, variant: "secondary", children: [_jsx("div", { className: "base-button__icon", children: page.step3refund.refund?.loading ? (_jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" })) : (_jsx("i", { className: 'icon icon-refund' })) }), "Refund"] }) })] }) }));
    }
    if (page.step4) {
        return (_jsxs(_Fragment, { children: [page.step4.state === 'expired' && swapFees, _jsxs("div", { className: "swap-panel__card", children: [page.step4.state !== 'expired' && stepByStep, _jsx(SwapStepAlert, { show: page.step4.state === 'refunded', type: "info", icon: ic_settings_backup_restore_outline, title: "Funds returning", description: "Funds refunded successfully!" }), _jsx(SwapStepAlert, { show: page.step4.state === 'success', type: "success", icon: ic_check_circle, title: "Swap success", description: "Your swap was executed successfully!", action: props.quote.getType() === SwapType.TO_BTC
                                ? {
                                    type: 'link',
                                    text: 'View transaction',
                                    href: FEConstants.btcBlockExplorer + props.quote.getOutputTxId(),
                                }
                                : undefined })] }), _jsx(BaseButton, { onClick: props.refreshQuote, variant: "primary", className: "swap-panel__action", children: "New Swap" })] }));
    }
}
