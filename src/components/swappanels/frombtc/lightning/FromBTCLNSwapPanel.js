...
export function FromBTCLNSwapPanel(props) {
    ...
    if (page.step3claim) {
        return (_jsx(_Fragment, { children: _jsxs("div", { className: "swap-panel__card", children: [stepByStep, _jsx(SwapStepAlert, { show: !!page.step3claim.error, type: "error", icon: ic_warning, title: page.step3claim.error?.title, error: page.step3claim.error?.error }), _jsx(SwapStepAlert, { show: true, type: "success", icon: ic_check_circle, title: "Lightning network payment received", description: "Claim your payment to finish the swap.", actionElement: _jsxs(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), className: "swap-step-alert__button", chainId: props.quote?.chainIdentifier, onClick: page.step3claim.commit.onClick, disabled: page.step3claim.commit.disabled, variant: "secondary", children: [page.step3claim.commit.loading ? (_jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" })) : (_jsx("i", { className: "icon icon-claim" })), "Claim your payment"] }) })] }) }));
    }
    if (page.step4) {
