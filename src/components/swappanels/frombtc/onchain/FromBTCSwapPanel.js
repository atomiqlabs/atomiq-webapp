...
export function FromBTCSwapPanel(props) {
    ...
    if (page.step1init) {
        return (_jsxs(_Fragment, { children: [swapFees, _jsx(SwapStepAlert, { show: !!page.step1init.error, type: "error", icon: ic_warning, title: page.step1init.error?.title, error: page.step1init.error?.error }), gasAlert, !!page.step1init.init && (_jsxs(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), chainId: props.quote.chainIdentifier, onClick: page.step1init.init.onClick, disabled: page.step1init.init.disabled, size: "lg", className: "swap-panel__action", children: [page.step1init.init.loading ? (_jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" })) : (''), "Swap"] }))] }));
    }
