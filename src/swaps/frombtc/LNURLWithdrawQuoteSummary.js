...
export function LNURLWithdrawQuoteSummary(props) {
    ...
    return (_jsxs(_Fragment, { children: [isInitiated ? _jsx(StepByStep, { steps: executionSteps }) : "", _jsx(SwapExpiryProgressBar, { expired: isQuoteExpired, timeRemaining: quoteTimeRemaining, totalTime: totalQuoteTime, show: (isClaimable || isQuoteExpiredClaim) &&
                    ...
                    smartChainWallet !== undefined }), isCreated || isClaimable ? (smartChainWallet === undefined ? (_jsx(ButtonWithWallet, { chainId: props.quote.chainIdentifier, requiredWalletAddress: props.quote._getInitiator(), size: "lg" })) : (_jsxs(_Fragment, { children: [_jsx(ErrorAlert, { className: "mb-3", title: "Swap " +
                            ...
                            " error", error: claimError ?? commitError ?? paymentError }), _jsxs(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), chainId: props.quote?.chainIdentifier, onClick: () => (isClaimable ? onCommit() : waitForPayment()), disabled: committing ||
                            paymentWaiting ||
                            (!canClaimInOneShot && isClaimClaimable), size: canClaimInOneShot ? "lg" : isClaimClaimable ? "sm" : "lg", children: [committing || paymentWaiting ? (_jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" })) : (""), canClaimInOneShot
                                ? "Claim"
                                : isClaimClaimable
                                    ? "1. Initialized"
                                    : committing
                                        ? "1. Initializing..."
                                        : "1. Initialize swap"] }), !canClaimInOneShot ? (_jsxs(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), chainId: props.quote?.chainIdentifier, onClick: () => onClaim(), disabled: claiming || !isClaimClaimable, size: isClaimClaimable ? "lg" : "sm", className: "mt-2", children: [claiming ? (_jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" })) : (""), claiming
                                ? "2. Claiming funds..."
                                : "2. Finish swap (claim funds)"] })) : ("")] }))) : (""), isSuccess ? (_jsxs(Alert, { variant: "success", className: "mb-0", children: [_jsx("strong", { children: "Swap successful" }), _jsx("label", { children: "Swap was executed successfully" })] })) : (""), isFailed ? (_jsxs(Alert, { variant: "danger", className: "mb-0", children: [_jsx("strong", { children: "Swap failed" }), _jsx("label", { children: "Swap HTLC expired, your lightning payment will be refunded shortly!" })] })) : (""), isQuoteExpired || isFailed || (isSuccess && props.type !== "payment") ? (_jsx(Button, { onClick: props.refreshQuote, variant: "secondary", children: "New quote" })) : ("")] }));
}
