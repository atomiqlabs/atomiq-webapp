...
export function FromBTCLNQuoteSummary(props) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(LightningHyperlinkModal, { openRef: openModalRef, hyperlink: props.quote.getHyperlink(), setShowHyperlinkWarning: setShowHyperlinkWarning }), _jsxs("div", { className: "swap-panel__card", children: [isInitiated ? (_jsx(StepByStep, { steps: executionSteps, sourceWallet: sourceWallet, destinationWallet: destinationWallet })) : null, _jsx(SwapStepAlert, { show: !!paymentError, type: "error", icon: ic_warning, title: "Swap initialization error", description: paymentError?.message || paymentError?.toString(), error: paymentError }), isCreated && paymentWaiting ? (_jsxs(_Fragment, { children: [_jsx(LightningQR, { quote: props.quote, payInstantly: initClicked, setAutoClaim: setAutoClaim, autoClaim: autoClaim, onHyperlink: onHyperlink }), _jsx("div", { className: "swap-panel__card__group", children: _jsx(SwapExpiryProgressBar, { timeRemaining: quoteTimeRemaining, totalTime: totalQuoteTime, show: true, type: "bar", expiryText: "Swap address expired, please do not send any funds!", quoteAlias: "Lightning invoice" }) })] })) : (''), _jsx(SwapStepAlert, { show: isSuccess, type: "success", icon: ic_check_circle, title: "Swap success", description: "Your swap was executed successfully!" }), _jsx(SwapStepAlert, { show: isFailed, type: "danger", icon: ic_warning, title: "Swap failed", description: "Swap HTLC expired, your lightning payment will be refunded shortly!" }), _jsx(SwapStepAlert, { show: !!(commitError || claimError), type: "error", icon: ic_warning, title: 'Swap ' +
                            (canClaimInOneShot || claimError != null ? 'claim' : ' claim initialization') +
                            ' error', description: (commitError ?? claimError)?.message || (commitError ?? claimError)?.toString(), error: commitError ?? claimError })] }), isCreated && paymentWaiting && (_jsx(BaseButton, { onClick: props.abortSwap, variant: "danger", className: "swap-panel__action is-large", children: "Abort swap" })), isClaimable ? (_jsxs(_Fragment, { children: [_jsxs(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), className: "swap-panel__action", chainId: props.quote?.chainIdentifier, onClick: () => onCommit(), disabled: committing || (!canClaimInOneShot && !isClaimCommittable), size: canClaimInOneShot ? 'lg' : isClaimCommittable ? 'lg' : 'sm', children: [committing ? _jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" }) : '', canClaimInOneShot
                                ? 'Finish swap (claim funds)'
                                : !isClaimCommittable
                                    ? '1. Initialized'
                                    : committing
                                        ? '1. Initializing...'
                                        : '1. Finish swap (initialize)'] }), !canClaimInOneShot ? (_jsxs(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), chainId: props.quote?.chainIdentifier, onClick: () => onClaim(), disabled: claiming || !isClaimClaimable, size: isClaimClaimable ? 'lg' : 'sm', className: "swap-panel__action", children: [claiming ? _jsx(Spinner, { animation: "border", size: "sm", className: "mr-2" }) : '', claiming ? '2. Claiming funds...' : '2. Finish swap (claim funds)'] })) : ('')] })) : (''), isCreated && !paymentWaiting ? (smartChainWallet === undefined ? (_jsx(ButtonWithWallet, { className: "swap-panel__action", chainId: props.quote.chainIdentifier, requiredWalletAddress: props.quote._getInitiator(), size: "lg" })) : (_jsxs(_Fragment, { children: [_jsx(SwapForGasAlert, { notEnoughForGas: props.notEnoughForGas, quote: props.quote }), _jsx(ButtonWithWallet, { requiredWalletAddress: props.quote._getInitiator(), chainId: props.quote?.chainIdentifier, className: "swap-panel__action", onClick: () => {
                            setInitClicked(true);
                            waitForPayment();
