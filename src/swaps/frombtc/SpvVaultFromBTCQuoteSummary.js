...
export function SpvVaultFromBTCQuoteSummary(props) {
  ...
  const [onSend, sendLoading, sendSuccess, sendError] = useAsync(() => {
    if (setAmountLockRef.current != null) {
      console.log(
        "SpvVaultFromBTCQuoteSummary: onSend(): setting amount lock to true",
      );
  ...
  const isReceived =
    state === SpvFromBTCSwapState.BROADCASTED && txData != null;
  const isClaimable =
    state === SpvFromBTCSwapState.BTC_TX_CONFIRMED && !claimLoading;
  const isClaiming =
    state === SpvFromBTCSwapState.BTC_TX_CONFIRMED && claimLoading;
  const isFailed =
  ...
  const isSuccess =
    state === SpvFromBTCSwapState.CLAIMED ||
    state === SpvFromBTCSwapState.FRONTED;
  useEffect(() => {
    if (isSuccess || isFailed || isQuoteExpired) {
      console.log(
        "SpvVaultFromBTCQuoteSummary: useEffect(state): setting amount lock to false",
      );
  ...
  return _jsxs(_Fragment, {
    children: [
      isInitiated || (isCreated && sendLoading)
        ? _jsx(StepByStep, { steps: executionSteps })
        : "",
      ...
      isBroadcasting
        ? _jsxs("div", {
            ...
            children: [
              ...
              _jsx("small", {
                className: "mt-2",
                children: "Sending bitcoin transaction...",
              }),
            ],
          })
        : "",
      ...
      isFailed
        ? _jsxs(Alert, {
            ...
            children: [
              ...
              _jsx("label", {
                children: "Swap transaction reverted, no funds were sent!",
              }),
            ],
          })
        : "",
