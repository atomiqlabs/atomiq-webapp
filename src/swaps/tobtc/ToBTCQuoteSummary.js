...
export function ToBTCQuoteSummary(props) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "swap-panel__card", children: [_jsx(SwapStepAlert, { show: isCreated && confidenceWarning, type: "warning", icon: ic_warning, title: "Payment might likely fail!", description: "We weren't able to check if the recipient is reachable (send probe request) on the Lightning network, this is common with some wallets, but could also indicate that the destination is unreachable and payment might therefore fail (you will get a refund in that case)!" }), _jsx(SwapStepAlert, { show: isCreated && nonCustodialWarning && props.type === 'swap', type: "info", icon: ic_check_circle, title: "Non-custodial wallet info", description: "Please make sure your lightning wallet is online & running to be able to receive a lightning network payment, otherwise the payment will fail (you will get a refund in that case)!" }), isInitiated ? (_jsxs(_Fragment, { children: [_jsx(StepByStep, { quote: props.quote, steps: executionSteps }), isSuccess ? (_jsx(SwapStepAlert, { type: "success", icon: ic_check_circle, title: "Swap success", description: "Your swap was executed successfully!", action: props.quote.getType() === SwapType.TO_BTC
                                    ? {
                                        type: 'link',
