...
function WrappedApp() {
  const { loading, loadingError: swapperError } = useContext(SwapperContext);
  const { loadError: chainsError } = useContext(ChainsContext);

  const isSwapperError = loading && !!swapperError;
  const isChainsError = !!chainsError;

  return (
    <>
      <MainNavigation />
      <main className="d-flex flex-grow-1 flex-column mt-4 mt-md-5">
        {(isSwapperError || isChainsError) && (
            ...
            <div className="mt-auto height-50 d-flex justify-content-center align-items-center flex-fill">
              <div className="text-white text-center">
                <ErrorAlert
                  ...
                  title={isSwapperError ? "atomiq network connection error" : "Website load error, please refresh the website"}
                  error={isSwapperError ? swapperError : chainsError}
