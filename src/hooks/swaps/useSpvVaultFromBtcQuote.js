...
export function useSpvVaultFromBtcQuote(quote, UICallback, feeRate, inputWalletBalance) {
    ...
    const [onWaitForPayment, waitingPayment, waitPaymentSuccess, waitPaymentError] = useAsync(() => {
        return quote.waitForBitcoinTransaction((txId, confirmations, confirmationTarget, txEtaMs) => {
            if (txId == null) {
                setTxData(null);
                return;
            }
    ...
    const step1init = useMemo(() => !isCreated
        ? undefined
        : {
    ...
    const step5 = useMemo(() => !isSuccess && !isFailed && !isQuoteExpired
        ? undefined
        : {
            state: isSuccess
                ? 'success'
