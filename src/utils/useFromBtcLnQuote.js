import { FromBTCLNSwapState, timeoutPromise } from "@atomiqlabs/sdk";
import { useContext, useEffect } from "react";
import { SwapsContext } from "../context/SwapsContext";
import { useSwapState } from "./useSwapState";
import { useStateRef } from "./useStateRef";
import { useAbortSignalRef } from "./useAbortSignal";
...
export function useFromBtcLnQuote(quote, setAmountLock) {
    ...
    const [onCommit, committing, commitSuccess, commitError] = useAsync(async (skipChecks) => {
        if (canClaimInOneShot) {
            ...
            if (quote.chainIdentifier === "STARKNET" && starknetWalletRef.current?.id === "argentX")
                await timeoutPromise(5000);
        }
    }, [quote, signer]);
    const [onClaim, claiming, claimSuccess, claimError] = useAsync((skipChecks) => quote.claim(signer), [quote, signer]);
    ...
    if (canClaimInOneShot) {
        ...
        if (isClaimCommittable)
            executionSteps[1] = { icon: ic_swap_horizontal_circle_outline, text: committing ? "Sending initialization transaction" : "Send initialization transaction", type: "loading" };
        if (isClaimClaimable) {
